const {
  scrape_whatsapp_chat,
} = require("../lib/GenericWhatsappAutomation/js/src/whatsapp_manager.js"); // Import the scraper function
...
async function handleMessage(message) {
  // send the message to a local python flusk server
  const axios = require("axios");
  const url = "http://gpt-service:5500/whatsapp";
  const data = {
...
async function main() {
  try {
    ...
    console.error(
      "Failed to load configuration or initialize WhatsApp scraper:",
      error
    );
  }
}
